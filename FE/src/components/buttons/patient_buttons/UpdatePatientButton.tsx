...
export default function SimpleModal() {
    ...
    const updatePatient = () => {
        axios.put("http://localhost:8080/patient",
            {sessionId: localStorage.getItem("sessionId"), idToUpdate: newPatient.id, name: newPatient.name, address: newPatient.address, gender: newPatient.gender, birthDate: newPatient.birthDate}).then(response => {
            window.location.reload();
        })
    };

    const isValidPatient = () => {
        return newPatient.address.length > 0 && newPatient.birthDate.length > 0 && newPatient.gender.length > 0 && newPatient.name.length > 0 && newPatient.id !== 0
    };
        ...
        <>
            ...
            >
                <Paper style={modalStyle} className={classes.paper}>
                    <Input placeholder="Id" onChange={e => newPatient.id = parseInt(e.target.value)}/> <br/>
